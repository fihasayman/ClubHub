    ...
    public partial class Admin_ADC_Pre : Form
    {
        ...
        private void Load_Pre_ACC_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(@"Data Source=DESKTOP-IILFUCP\SQLEXPRESS;Initial Catalog=UniversityClubDB;Integrated Security=True;Encrypt=False"))
            {
                SqlDataAdapter da = new SqlDataAdapter(
                    "SELECT ID, FirstName, LastName, Email, Phone,Password, Gender, StudentID, Department, Age FROM ADC_President", con);
        ...
        private void Update_Mem_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(@"Data Source=DESKTOP-IILFUCP\SQLEXPRESS;Initial Catalog=UniversityClubDB;Integrated Security=True;Encrypt=False"))
            {
                ...
                if (id == 0)
                {
                    string insertQuery = @"INSERT INTO ADC_President
...
SELECT SCOPE_IDENTITY()";
                ...
                {
                    string updateQuery = @"UPDATE ADC_President SET 
...
WHERE ID=@ID";
        ...
        private void Remove_Pre_ACC_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("DELETE FROM ADC_President WHERE ID=@ID", con);
