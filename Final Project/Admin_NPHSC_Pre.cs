...
namespace Final_Project
{
        ...
        private void Load_Pre_ACC_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(@"Data Source=DESKTOP-IILFUCP\SQLEXPRESS;Initial Catalog=UniversityClubDB;Integrated Security=True;Encrypt=False"))
            {
                SqlDataAdapter da = new SqlDataAdapter(
                    "SELECT ID, FirstName, LastName, Email, Phone,Password, Gender, StudentID, Department, Age FROM NSUPHSC_President", con);
    ...
    age <= 0)
            ...
            using (SqlConnection con = new SqlConnection(@"Data Source=DESKTOP-IILFUCP\SQLEXPRESS;Initial Catalog=UniversityClubDB;Integrated Security=True;Encrypt=False"))
            {
                ...
                if (id == 0)
                {
                    string insertQuery = @"INSERT INTO NSUPHSC_President
...
SELECT SCOPE_IDENTITY()";
                ...
                {
                    string updateQuery = @"UPDATE NSUPHSC_President SET 
...
WHERE ID=@ID";
        ...
        private void Remove_Pre_ACC_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("DELETE FROM NSUPHSC_President WHERE ID=@ID", con);
