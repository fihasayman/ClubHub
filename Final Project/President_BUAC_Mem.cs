...
{
    public partial class President_BUAC_Mem : Form
    {
        public President_BUAC_Mem()
        {
            InitializeComponent();
        }
        ...
        {
            President_BUAC_Event Event = new President_BUAC_Event();
            Event.Show();
            this.Hide();
        }
...
FROM BUAC_Member";
        ...
        private void Update_Mem_Click(object sender, EventArgs e)
        {
            ...
            string password = row["Gender"].ToString().Trim();
      ...
      Integrated Security=True;Encrypt=False"))
            {
                ...
                SqlCommand cmd = new SqlCommand(
                    "DELETE FROM dbo.BUAC_Member WHERE MemberID = @Id", con);
