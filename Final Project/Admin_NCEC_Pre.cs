...
{
    public partial class Admin_NCEC_Pre : Form
    {
        public Admin_NCEC_Pre()
        {
            InitializeComponent();
        }
        ...
        private void Load_Pre_ACC_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(@"Data Source=DESKTOP-IILFUCP\SQLEXPRESS;Initial Catalog=UniversityClubDB;Integrated Security=True;Encrypt=False"))
            {
                SqlDataAdapter da = new SqlDataAdapter(
                    "SELECT ID, FirstName, LastName, Email, Phone,Password, Gender, StudentID, Department, Age FROM NSUCEC_President", con);
        ...
        private void Update_Mem_Click(object sender, EventArgs e)
        {
            ...
            string phone = row["Phone"] == DBNull.Value ? "" : row["Phone"].ToString().Trim();
    ...
    age <= 0)
            ...
            using (SqlConnection con = new SqlConnection(@"Data Source=DESKTOP-IILFUCP\SQLEXPRESS;Initial Catalog=UniversityClubDB;Integrated Security=True;Encrypt=False"))
            {
                ...
                if (id == 0)
                {
                    string insertQuery = @"INSERT INTO NSUCEC_President
...
SELECT SCOPE_IDENTITY()";
                ...
                {
                    string updateQuery = @"UPDATE NSUCEC_President SET 
...
WHERE ID=@ID";
        ...
        private void Remove_Pre_ACC_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("DELETE FROM NSUCEC_President WHERE ID=@ID", con);
